...
from custom_admin.impact_normalization import compute_impact_metrics, qs_iter
...
def _impact_report_metrics_for_days(days: int) -> dict:
    """
    Build a small, reusable metric payload for exports.
    Kept intentionally minimal and stable across export formats.
    """
    start_date = timezone.now() - timedelta(days=days)

    # Canonical:
    # - environmental metrics: delivered food only
    # - monetary/subsidy: social/economic only
    delivered_food = Donation.objects.filter(
        donated_at__gte=start_date,
        status='accepted',
        delivery_status='delivered',
        donation_category='food',
    ).only('id', 'item_name', 'quantity', 'quantity_unit', 'donation_category')

    accepted_money = Donation.objects.filter(
        donated_at__gte=start_date,
        status='accepted',
        donation_category='monetary',
    ).only('id', 'amount', 'donation_category')

    accepted_subsidized = Donation.objects.filter(
        donated_at__gte=start_date,
        status='accepted',
        donation_type='subsidized',
    ).only('id', 'subsidized_market_price', 'subsidized_price', 'donation_type')
    ...
    metrics, _debug = compute_impact_metrics(
        days=days,
        start_date=start_date,
        delivered_food_donations=qs_iter(delivered_food),
        accepted_monetary_donations=qs_iter(accepted_money),
        accepted_subsidized_donations=qs_iter(accepted_subsidized),
        unique_recipients_helped=unique_recipients,
    )

    trees_equivalent = (metrics.co2_saved_tons * 1000 / 21.0) if metrics.co2_saved_tons else 0

    return {
        'days': days,
        'start_date': start_date,
        'total_donations': metrics.donations_count,
        'food_kg_delivered': metrics.food_kg_delivered,
        'food_waste_prevented': metrics.food_waste_prevented_kg,
        'total_co2_saved': metrics.co2_saved_tons,
        'meals_provided': metrics.meals_provided,
        'estimated_beneficiaries': metrics.estimated_beneficiaries,
        'trees_equivalent': float(trees_equivalent or 0),
        'monetary_amount_total': metrics.monetary_amount_total,
        'meals_funded': metrics.meals_funded,
        'subsidy_value_total': metrics.subsidy_value_total,
        'unknown_weight_donations_count': metrics.unknown_weight_donations_count,
    }
...
def environmental_impact_dashboard(request):
    ...
    # === CANONICAL POPULATION RULES ===
    # Environmental metrics: delivered food only (accepted + delivered)
    delivered_food_donations = Donation.objects.filter(
        donated_at__gte=start_date,
        status='accepted',
        delivery_status='delivered',
        donation_category='food',
    ).only('id', 'item_name', 'quantity', 'quantity_unit', 'donation_category', 'foodbank_id', 'donor_id')

    # Social/economic: accepted monetary + accepted subsidized (economic only)
    accepted_monetary_donations = Donation.objects.filter(
        donated_at__gte=start_date,
        status='accepted',
        donation_category='monetary',
    ).only('id', 'amount', 'donation_category')

    accepted_subsidized_donations = Donation.objects.filter(
        donated_at__gte=start_date,
        status='accepted',
        donation_type='subsidized',
    ).only('id', 'subsidized_market_price', 'subsidized_price', 'donation_type', 'delivery_status')

    # Keep an accepted-donations queryset for operational breakdown charts (non-environmental)
    all_accepted_donations = Donation.objects.filter(
        donated_at__gte=start_date,
        status='accepted',
    )
    ...
    donation_breakdown = {
        'free_food': all_accepted_donations.filter(donation_category='food', donation_mode='free').aggregate(
            count=Count('id'),
            total_quantity=Sum('quantity'),
        ),
        'subsidized_food': all_accepted_donations.filter(donation_category='food', donation_mode='subsidized').aggregate(
            count=Count('id'),
            total_quantity=Sum('subsidized_quantity'),
        ),
        'free_non_food': all_accepted_donations.filter(donation_category='non_food', donation_mode='free').aggregate(
            count=Count('id'),
            total_units=Sum('quantity'),
        ),
        'subsidized_non_food': all_accepted_donations.filter(donation_category='non_food', donation_mode='subsidized').aggregate(
            count=Count('id'),
            total_units=Sum('subsidized_quantity'),
        ),
        'monetary': all_accepted_donations.filter(donation_category='monetary').aggregate(
            count=Count('id'),
            total_amount=Sum('amount'),
        ),
        'csr_others': all_accepted_donations.filter(donation_category__in=['csr', 'other']).aggregate(
            count=Count('id'),
            total_units=Sum('quantity'),
        ),
    }
    ...
    metrics, _debug_rows = compute_impact_metrics(
        days=days,
        start_date=start_date,
        delivered_food_donations=qs_iter(delivered_food_donations),
        accepted_monetary_donations=qs_iter(accepted_monetary_donations),
        accepted_subsidized_donations=qs_iter(accepted_subsidized_donations),
        unique_recipients_helped=unique_recipients,
    )

    food_waste_prevented = metrics.food_waste_prevented_kg
    co2_saved_food = metrics.co2_saved_tons
    total_co2_saved = metrics.co2_saved_tons

    # Optional: keep non-food as unit totals only (no CO₂ in canonical methodology)
    ...
    total_donations_count = all_accepted_donations.count()
    
    foodbank_stats = all_accepted_donations.values('foodbank__foodbank_name').annotate(
    ...
    foodbank_impact = []
    foodbank_impact_stats = all_accepted_donations.values('foodbank__foodbank_name').annotate(
    ...
    for fb_stat in foodbank_impact_stats:
        ...
        # Canonical (first iteration): food-based environmental impact is computed from delivered donations only.
        # For now we keep the per-foodbank impact bar as a proxy based on accepted food quantities.
        food_waste_prevented_fb = calculate_food_waste_prevented(free_food_fb, subsidized_food_fb)
        co2_saved_total_fb = float(calculate_co2_saved(food_waste_prevented_fb))
        waste_prevented_total_fb = float(food_waste_prevented_fb)
    ...
    total_monetary_value = metrics.monetary_amount_total
    
    economic_impact = calculate_economic_impact(monetary_donations_count, csr_donations_count)
    
    # === DONOR & CSR ANALYTICS ===
    # Contributions by donor type
    donor_type_breakdown = all_accepted_donations.values('donor__user_type').annotate(
        count=Count('id'),
        total_amount=Sum('amount')
    )
    
    # CSR subcategory breakdown
    csr_breakdown = all_accepted_donations.filter(
    ...
    request_linked_donations = all_accepted_donations.filter(foodbank_request__isnull=False).count()
    general_donations = all_accepted_donations.filter(foodbank_request__isnull=True).count()
    ...
    environmental_chart_labels = ['Food Waste Prevented (kg)', 'CO₂ Saved (tons)']
    ...
    context = {
        ...
        'non_food_waste_prevented': 0,
        'co2_saved_non_food': 0,
        'total_co2_saved': total_co2_saved,
        'total_waste_prevented': food_waste_prevented,
        ...
        # Raw (accepted) metrics for detailed breakdown
        'free_food_kg': donation_breakdown['free_food'].get('total_quantity') or 0,
        'subsidized_food_kg': donation_breakdown['subsidized_food'].get('total_quantity') or 0,
        ...
        # Top-of-page summary: canonical metrics (so UI matches exports)
        'completed_donations_total': metrics.donations_count,
        'completed_food_waste_prevented': metrics.food_waste_prevented_kg,
        'completed_total_co2_saved': metrics.co2_saved_tons,
    }
...
def export_impact_report_pdf(request):
    ...
    days = int(request.GET.get('days', 365))
    metrics_payload = _impact_report_metrics_for_days(days)
    start_date = metrics_payload['start_date']

    food_waste_prevented = float(metrics_payload['food_waste_prevented'] or 0)
    total_co2_saved = float(metrics_payload['total_co2_saved'] or 0)
    meals_provided = int(metrics_payload['meals_provided'] or 0)
    estimated_beneficiaries = int(metrics_payload['estimated_beneficiaries'] or 0)
    total_donations = int(metrics_payload['total_donations'] or 0)

    monetary_amount_total = metrics_payload.get('monetary_amount_total')
    subsidy_value_total = metrics_payload.get('subsidy_value_total')
    meals_funded = int(metrics_payload.get('meals_funded') or 0)
    unknown_weight_donations_count = int(metrics_payload.get('unknown_weight_donations_count') or 0)
    ...
    summary_data = [
        ['Metric', 'Value', 'Impact'],
        ['Total CO₂ Saved', f'{total_co2_saved} tons', 'Climate Action'],
        ...
        ['Delivered food donations', f'{total_donations:,.0f}', 'Community Engagement'],
        ['Unknown-weight donations', f'{unknown_weight_donations_count:,.0f}', 'Needs item catalog mapping'],
    ]
    ...
    donation_data = [
        ...
        ['Delivered food (normalized)', f"{total_donations:,}", f"{metrics_payload.get('food_kg_delivered', 0):,.0f} kg", "Direct waste prevention"],
        ['Monetary (accepted)', '-', f"KES {monetary_amount_total:,.0f}" if monetary_amount_total is not None else "KES 0", f"Meals funded (est.): {meals_funded:,}"],
        ['Subsidy value (est.)', '-', f"KES {subsidy_value_total:,.0f}" if subsidy_value_total is not None else "KES 0", "Economic relief (not CO₂)"],
    ]
    ...
    env_data = [
        ...
        ['CO₂ Emissions Saved', f'{total_co2_saved} tons', f'{trees_equivalent:,} trees planted'],
        ['Carbon Footprint Reduction', f'{total_co2_saved} tons CO₂', f'{cars_equivalent} cars off road/year'],
    ]
    ...
    <b>SDG 13 (Climate Action):</b> {total_co2_saved} tons CO₂ saved<br/>
...
@staff_member_required
def export_impact_report_pptx(request):
    """
    Export Wingu Impact Report as a real PowerPoint (.pptx).
    """
    try:
        ...
        return HttpResponse(
            "PowerPoint export requires python-pptx. Please install it and retry.",
            status=500,
    ...
    metrics = _impact_report_metrics_for_days(days)

    prs = Presentation()

    # Slide 1 - Title
    slide = prs.slides.add_slide(prs.slide_layouts[0])
    slide.shapes.title.text = "Wingu Impact Report"
    slide.placeholders[1].text = f"Period: last {metrics['days']} days"
    ...
    tf.clear()
    p = tf.paragraphs[0]
    p.text = f"CO₂ saved: {metrics['total_co2_saved']:.2f} tons"
    p.font.size = Pt(20)

    for line in [
        f"Food waste prevented: {metrics['food_waste_prevented']:.0f} kg",
        f"Meals provided (est.): {metrics['meals_provided']:,}",
        f"Beneficiaries supported (est.): {metrics['estimated_beneficiaries']:,}",
        f"Trees planted equivalent (est.): {metrics['trees_equivalent']:.0f}",
    ]:
    ...
    # Slide 3 - Donation breakdown
    slide = prs.slides.add_slide(prs.slide_layouts[1])
    slide.shapes.title.text = "Donation Breakdown"
    tf = slide.shapes.placeholders[1].text_frame
    tf.clear()
    rows = [
        ("Free food", metrics['donation_breakdown']['free_food']['count'], metrics['donation_breakdown']['free_food']['total_kg'], "kg"),
        ("Subsidized food", metrics['donation_breakdown']['subsidized_food']['count'], metrics['donation_breakdown']['subsidized_food']['total_kg'], "kg"),
        ("Monetary", metrics['donation_breakdown']['monetary']['count'], metrics['donation_breakdown']['monetary']['total_amount'], "KES"),
        ("CSR & Other", metrics['donation_breakdown']['csr_others']['count'], metrics['donation_breakdown']['csr_others']['total_units'], "units"),
    ]
    for label, count, total, unit in rows:
        para = tf.add_paragraph()
        if unit == "KES":
            para.text = f"{label}: {count} donations, total KES {float(total or 0):,.0f}"
        else:
            para.text = f"{label}: {count} donations, total {float(total or 0):,.0f} {unit}"
        para.level = 0
    ...
    response['Content-Disposition'] = (
        f'attachment; filename="Wingu_Impact_Report_{days}days_{timezone.now().strftime("%Y%m%d")}.pptx"'
    )
    return response


@staff_member_required
def export_impact_social_card_png(request):
    """
    Export a social media card as PNG (1200x630).
    """
    try:
        from PIL import Image, ImageDraw, ImageFont
    except Exception:
        return HttpResponse(
            "Social card export requires Pillow. Please install it and retry.",
            status=500,
            content_type="text/plain",
        )
    ...
    def _center(text: str, y: int, font):
        bbox = draw.textbbox((0, 0), text, font=font)
        tw = bbox[2] - bbox[0]
        draw.text(((width - tw) // 2, y), text, fill="white", font=font)

    _center("Wingu Impact Report", 60, font_title)
    _center(f"Last {days} days", 95, font_body)

    lines = [
        f"CO₂ saved: {metrics['total_co2_saved']:.2f} tons",
        f"Food waste prevented: {metrics['food_waste_prevented']:.0f} kg",
        f"Meals provided (est.): {metrics['meals_provided']:,}",
        f"Beneficiaries supported (est.): {metrics['estimated_beneficiaries']:,}",
    ]
    ...
    response['Content-Disposition'] = (
        f'attachment; filename="Wingu_Impact_Social_Card_{days}days_{timezone.now().strftime("%Y%m%d")}.png"'
    )
    return response


@staff_member_required
def export_impact_infographic_png(request):
    """
    Export a simple infographic as PNG (1080x1920).
    """
    try:
        from PIL import Image, ImageDraw, ImageFont
    except Exception:
        return HttpResponse(
            "Infographic export requires Pillow. Please install it and retry.",
            status=500,
            content_type="text/plain",
        )
    ...
    img = Image.new("RGB", (width, height), (5, 150, 105))
    draw = ImageDraw.Draw(img)
    font_title = ImageFont.load_default()
    font_body = ImageFont.load_default()
    ...
    _center("Wingu Impact Infographic", 80, font_title)
    _center(f"Last {days} days", 120, font_body)

    blocks = [
        ("CO₂ saved (tons)", f"{metrics['total_co2_saved']:.2f}"),
        ("Food waste prevented (kg)", f"{metrics['food_waste_prevented']:.0f}"),
        ("Meals provided (est.)", f"{metrics['meals_provided']:,}"),
        ("Beneficiaries supported (est.)", f"{metrics['estimated_beneficiaries']:,}"),
        ("Trees equivalent (est.)", f"{metrics['trees_equivalent']:.0f}"),
    ]

    y = 260
    for label, value in blocks:
        draw.rectangle([80, y, width - 80, y + 220], outline=(255, 255, 255), width=4)
        _center(label, y + 40, font_body)
        _center(value, y + 110, font_title)
        y += 260
    ...
    response = HttpResponse(out.getvalue(), content_type="image/png")
    response['Content-Disposition'] = (
        f'attachment; filename="Wingu_Impact_Infographic_{days}days_{timezone.now().strftime("%Y%m%d")}.png"'
    )
    return response
