...
class AdminRegistrationForm(UserCreationForm):
    ...
    def clean_admin_code(self):
        ...
        try:
            code_obj = AdminCode.objects.get(code=admin_code, is_active=True)
            # Store the code object for later use in save method
            self._admin_code_obj = code_obj
            return admin_code
        except AdminCode.DoesNotExist:
            raise forms.ValidationError('Invalid admin authorization code. Access denied.')
    ...
    def save(self, commit=True):
        ...
        if commit:
            ...
            if hasattr(self, '_admin_code_obj'):
                self._admin_code_obj.mark_as_used(user)
